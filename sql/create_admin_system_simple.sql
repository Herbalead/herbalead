-- Script SIMPLES para criar sistema administrativo
-- Execute este código no Supabase SQL Editor PASSO A PASSO

-- PASSO 1: Criar tabela courses (se não existir)
...
-- PASSO 3: Criar tabela course_materials (se não existir)
...
-- PASSO 4: Adicionar campo is_admin na tabela professionals
ALTER TABLE professionals ADD COLUMN IF NOT EXISTS is_admin BOOLEAN DEFAULT false;

-- PASSO 5: Criar bucket para materiais (se não existir)
...
-- PASSO 7: Criar políticas básicas
...
CREATE POLICY IF NOT EXISTS "course_modules_select_all" ON course_modules
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "course_modules_admin_all" ON course_modules
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM professionals p 
      WHERE p.id = auth.uid() 
...
CREATE POLICY IF NOT EXISTS "course_materials_select_active" ON course_materials
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM professionals p 
      WHERE p.id = auth.uid() 
...
CREATE POLICY IF NOT EXISTS "course_materials_admin_all" ON course_materials
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM professionals p 
      WHERE p.id = auth.uid() 
...
-- PASSO 10: Inserir módulos iniciais
