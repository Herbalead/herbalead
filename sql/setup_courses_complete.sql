-- SCRIPT COMPLETO PARA CONFIGURAR CURSOS AUTOMATICAMENTE
-- Execute este código no Supabase SQL Editor

-- 1. Limpar dados existentes (opcional)
-- DELETE FROM course_modules WHERE course_id IN (SELECT id FROM courses WHERE title IN ('Treinamento Herbalead', 'Manual de Vendas'));
-- DELETE FROM courses WHERE title IN ('Treinamento Herbalead', 'Manual de Vendas');

-- 2. Criar curso principal "Treinamento Herbalead"
INSERT INTO courses (
  id,
  title,
  description,
...
-- 3. Criar curso "Manual de Vendas" (separado)
INSERT INTO courses (
  id,
  title,
  description,
...
-- 4. Criar módulos para "Treinamento Herbalead"
...
-- 5. Criar módulo para "Manual de Vendas"
...
SELECT 
  ...
  c.duration_hours as duracao_horas,
  c.is_active as ativo,
  COUNT(cm.id) as total_modulos
FROM courses c
LEFT JOIN course_modules cm ON c.id = cm.course_id
WHERE c.title IN ('Treinamento Herbalead', 'Manual de Vendas')
GROUP BY c.id, c.title, c.description, c.duration_hours, c.is_active
ORDER BY c.title;

-- 7. Verificar módulos criados
SELECT 
  c.title as curso,
  cm.title as modulo,
...
-- 8. Instruções para upload manual dos arquivos HTML
-- 
