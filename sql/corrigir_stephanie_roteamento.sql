-- Script para corrigir o problema de roteamento da Stephanie Izidio
-- Problema: links.user_id referencia auth.users.id mas sistema busca professionals.id

-- 1. Encontrar o professional.id da Stephanie
SELECT 
    id,
    name,
    email
...
-- 2. Verificar os links atuais da Stephanie
SELECT 
    l.id,
    l.name,
    l.user_id,
...
-- 3. CORREÇÃO: Atualizar os links para usar professional.id como user_id
-- Substitua 'PROFESSIONAL_ID_AQUI' pelo ID encontrado na consulta 1

-- Primeiro, vamos deletar os links incorretos
DELETE FROM links 
WHERE user_id = 'd689279a-2ecd-4a58-a84d-f5eb78cd9e71'
  AND name IN ('proteina', 'minhaproteina');

-- 4. Criar os links corretos usando professional.id
-- (Execute primeiro a consulta 1 para encontrar o professional.id)
-- Substitua 'PROFESSIONAL_ID_AQUI' pelo ID real encontrado
...
(
    gen_random_uuid(),
    'PROFESSIONAL_ID_AQUI', -- Substitua pelo professional.id da Stephanie
    'proteina',
    'protein-calculator',
...
-- 6. Testar as URLs após a correção
