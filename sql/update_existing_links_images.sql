-- Script para atualizar links existentes com imagens OG
-- Aplicar sistema simples: ferramenta = imagem fixa

-- 1. Adicionar coluna og_image se não existir
ALTER TABLE links ADD COLUMN IF NOT EXISTS og_image TEXT;

-- 2. Atualizar links existentes com imagens baseadas na ferramenta
UPDATE links 
SET og_image = CASE 
...
-- 4. Contar links por ferramenta
SELECT 
    tool_name,
    COUNT(*) as total_links,
    COUNT(CASE WHEN og_image IS NOT NULL AND og_image != '' THEN 1 END) as links_with_image
FROM links
GROUP BY tool_name
