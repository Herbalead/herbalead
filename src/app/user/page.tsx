...
const supabase = createClient(supabaseUrl, supabaseKey)
  ...
  // Função para selecionar automaticamente a ferramenta baseada no nome do projeto
  const getAutoSelectedTool = (projectName: string) => {
    const name = projectName.toLowerCase()
    
    // Mapeamento de palavras-chave para ferramentas
    if (name.includes('quiz') || name.includes('perfil') || name.includes('teste')) {
      return 'perfil-bem-estar'
    }
    if (name.includes('bmi') || name.includes('imc') || name.includes('peso')) {
      return 'bmi'
    }
    if (name.includes('proteina') || name.includes('proteína') || name.includes('nutrição') || name.includes('nutricao')) {
      return 'protein'
    }
    if (name.includes('composição') || name.includes('composicao') || name.includes('corporal')) {
      return 'body-composition'
    }
    if (name.includes('refeição') || name.includes('refeicao') || name.includes('plano') || name.includes('cardápio') || name.includes('cardapio')) {
      return 'meal-planner'
    }
    if (name.includes('hidratação') || name.includes('hidratacao') || name.includes('água') || name.includes('agua')) {
      return 'hydration'
    }
    if (name.includes('avaliação') || name.includes('avaliacao') || name.includes('nutricional')) {
      return 'nutrition-assessment'
    }
    if (name.includes('objetivo') || name.includes('meta') || name.includes('saúde') || name.includes('saude')) {
      return 'health-goals'
    }
    if (name.includes('desafio') || name.includes('7 dias') || name.includes('semana')) {
      return 'desafio-7-dias'
    }
    if (name.includes('aproveitando') || name.includes('100%') || name.includes('cliente')) {
      return 'aproveitando-100'
    }
    if (name.includes('empreendedor') || name.includes('recrutamento') || name.includes('negócio') || name.includes('negocio')) {
      return 'perfil-empreendedor'
    }
    if (name.includes('inspirar') || name.includes('liderança') || name.includes('lideranca')) {
      return 'inspirar-pessoas'
    }
    if (name.includes('onboarding') || name.includes('rápido') || name.includes('rapido')) {
      return 'onboarding-rapido'
    }
    if (name.includes('bem-estar') || name.includes('bem estar') || name.includes('diário') || name.includes('diario')) {
      return 'bem-estar-diario'
    }
    if (name.includes('alimentação') || name.includes('alimentacao') || name.includes('saudável') || name.includes('saudavel')) {
      return 'alimentacao-saudavel'
    }
    
    // Default para projetos gerais
    return 'perfil-bem-estar'
  }

  // Função para verificar se o nome do projeto já existe
  ...
  const createCustomLink = async () => {
    ...
    if (!newLink.project_name?.trim()) {
      console.error('❌ Nome do projeto não informado')
      alert('Por favor, informe o nome do projeto')
      return
    }
    
    // Verificar se o nome do projeto já existe
    const projectNameExists = await checkProjectNameExists(newLink.project_name.trim())
    if (projectNameExists) {
      alert('Este nome de projeto já existe. Por favor, escolha um nome único.')
      return
    }
    ...
    try {
      // Selecionar ferramenta automaticamente baseada no nome do projeto
      const autoSelectedTool = getAutoSelectedTool(newLink.project_name)
      console.log('🤖 Ferramenta selecionada automaticamente:', autoSelectedTool)
      ...
      const projectDomain = user.project_id || 'herbalead' // Default para Herbalead
      const baseDomain = process.env.NEXT_PUBLIC_BASE_DOMAIN || 'herbalead.com'
      const protocol = process.env.NODE_ENV === 'production' ? 'https' : 'http'
      const customUrl = `${protocol}://${baseDomain}/link/${customSlug}`
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setNewLink({...newLink, project_name: projectName})
                          }}
                          placeholder="Ex: Campanha Instagram, Leads Nutrição, Vendas Q1"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                        />
                        <p className="text-xs text-gray-500 mt-1">Dê um nome único para identificar esta estratégia</p>
                      </div>
                      
                      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                        <h4 className="text-sm font-medium text-emerald-900 mb-2">🤖 Seleção Automática</h4>
                        <p className="text-xs text-emerald-700">
                          A ferramenta será selecionada automaticamente baseada no nome do projeto.<br/>
                          A URL será gerada automaticamente: herbalead.com/link/nome-automatico
                        </p>
                      </div>
